...
%    epsConverce, epsilon in convergence criterion (around 1e-5)
% --warm start--
% tau_warmStart, store to increase speed in the next execution 
% nu_warmStart, store to increase speed in the next execution 
% use_warmStart, boolean (use warm start or not)
%
% Outputs:
...
%  steps, number of iterations till convergence
%  error, error between muLast and mu -> convergence criteria
%  mu_last_iteration, last muIN -> needed for warm start
%  sigma_last_iteration, last sigmaIN -> needed for warm start
%%%%%%%%%%%%%%%%%%%%%%%
function [logZEP, mu, Sigma, steps, error,  last_tauCavity, last_nuCavity] = axisepmgp_warmStart(m,K,lowerB,upperB, maxSteps, epsConverge, tau_warmStart, nu_warmStart, use_warmStart)
  mu_last_iteration = 0; 
  sigma_last_iteration = 0;
  ...
  steps = 0; %steps till convergence needed 
  ...
  while(doLoop)
    ...
    if( steps==1 && use_warmStart)
        tauCavity = tau_warmStart;
        nuCavity = nu_warmStart;
    elseif( steps==1 && ~use_warmStart)
        tauCavity = 1./diag(Sigma) - tauSite;
        nuCavity = mu./diag(Sigma) - nuSite;
    end
    
    muIN = nuCavity./tauCavity;
    sigmaIN = 1./tauCavity;
    [logZhat, Zhat, muhat, sighat] = truncNormMoments(lowerB,upperB,muIN,sigmaIN);
    ...
    tauCavity = 1./diag(Sigma) - tauSite;
    nuCavity = mu./diag(Sigma) - nuSite;
    last_tauCavity = tauCavity;
    last_nuCavity = nuCavity;
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    error = norm(muLast-mu);
    
    if ((error) < epsConverge)
      converged=1;
    else
      muLast = mu;
    end
